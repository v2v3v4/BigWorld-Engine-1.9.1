 ...
 */
static void debugDropFilter(PyObject * pObject)
{
	...
	if (pObject != NULL)
	{
		if ( !AvatarDropFilter::Check( pObject ) )
		{
		...
		g_debugFilter = static_cast<AvatarDropFilter *>( pObject );
	}
}

PY_AUTO_MODULE_FUNCTION(
	RETVOID, debugDropFilter, OPTARG(PyObject *, NULL, END), BigWorld )
