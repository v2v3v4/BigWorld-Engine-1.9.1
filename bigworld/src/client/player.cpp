...
bool Player::drawPlayer( Moo::RenderTargetPtr renderTexture, 
							bool useDepth /*=false */ )
{
	BW_GUARD;
	...
	if (pPlayer && pPlayer->pPrimaryModel())
	{
		...
		if ( pChunk )
		{
			...
			if ( useDepth )
			{
				DX::Surface* depthBuffer = renderTexture->depthBuffer();
			...
			// DrawPlayer can be called multiple times (for example in distortion buffer), so
			// we shouldn't clear sorted channel yet.
