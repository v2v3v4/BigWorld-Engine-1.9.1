		...
		if (pDebutant->incrementEnter() > 0)
		{
				...
				pDebutant->isClientOnly())
			{
				...
				// to be changed in the future.
					MF_ASSERT_DEV( vehicleID == 0 && "Client only entities should " \
	...
	const std::string &desc, BinaryIStream & data )
{
	...
	bool callbackDone = Script::call(
		PyObject_GetAttrString( Personality::instance(), "onStreamComplete" ),
	...
	if (!callbackDone && PyObject_HasAttrString( Personality::instance(),
