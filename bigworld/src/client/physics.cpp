...
void Physics::avatarStyleTick( double timeNow, double timeLast )
{
	...
	if (fall_)
	{
		droppingTime_ = this->applyGravity(vehicleID, oldPos, newPos,
			mc, dTime);

		this->applyBuoyancy(newPos, dTime);
	}
	...
	this->scaleUphillSpeed(maxDistance, oldPos, newPos);
	...
	float localAuxVolatile[3] = {mc.modelYaw + newHeadYaw,
		mc.modelPitch + newHeadPitch, 0};
...
{
	BW_GUARD;
	bool usingKeyboard = InputDevices::joystick().isUsingKeyboard() ||
		tempJoystickDisabled_;
	tempJoystickDisabled_ = false;
...
/**
 *	This method applies buoyancy, if the player is underwater.
 *
 *	@param	newPos		[out]Resultant position
 *	@param	dTime		delta time
 */
void Physics::applyBuoyancy( Vector3& newPos, float dTime )
{
	BW_GUARD;
	if (inWater_)
	{
		if (newPos.y < waterSurfaceHeight_)
		{
			droppingTime_ -= dTime * viscosity_;
			droppingTime_ = max( 0.f, droppingTime_ );
			float buoyancy = buoyancy_ * (1.f - min(droppingTime_,1.f));
			newPos.y += dTime * buoyancy;
		}

		if (newPos.y > waterSurfaceHeight_)
		{
			newPos.y = waterSurfaceHeight_;
		}
	}
}
 ...
 *	@param	newPos			[out]Resultant position.
 */
void Physics::scaleUphillSpeed( float maxDistance, const Vector3& oldPos, Vector3& newPos )
{	
	BW_GUARD;
 ...
 */
float Physics::applyGravity( EntityID & vehicleID, const Vector3 & oldPos, Vector3 & newPos,
	const MotionConstants & mc, float deltaTime ) const
{
	BW_GUARD;
	const EntityID oldVehicleID = vehicleID;
...
PY_BEGIN_ATTRIBUTES( Physics )
	 ...
	 *	damped when it has entered water (set by the inWater flag).
	 *
	 *	@type	Float
	 */
