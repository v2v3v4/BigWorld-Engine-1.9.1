					...
					const PGVector& newPrimGroups )
{
	if (pPrimFile_)
	{
		...
		return pPrimFile_->writeBinary( vertGroup_, qfw.output() );
	}
...
{	
	...
	return this->savePrimitiveFile< BumpedVertexType >( bumpedVertices, newIndices, newPrimGroups );
}
...
bool VisualBumper::updatePrimitives() 
{
	...
	if (std::string("xyznuv") == format_)
	{
		...
		beenBumped = this->makeBumped< Moo::VertexXYZNUV, Moo::VertexXYZNUVTB >( 
			verts, vh->nVertices_, sourceIndices, primGroups, hasMorphTargets );
	}
