...
void Mutant::batched( bool val, bool undo /*=true*/ )
{
	...
	if (undo)
	{
		...
		UndoRedo::instance().barrier( L("MODELEDITOR/MODELS/MUTANT/CHANGING_BATCH"), false );
	}
...
void Mutant::dpvsOccluder( bool val )
{
	...
	UndoRedo::instance().add( new UndoRedoOp( 0, data, currModel_ ));
	UndoRedo::instance().barrier( L("MODELEDITOR/MODELS/MUTANT/CHANGING_OCCLUDER"), false );
...
void Mutant::editorProxyName( std::string editorProxyName )
{
	...
	if (data == NULL) data = currModel_;
	UndoRedo::instance().add( new UndoRedoOp( 0, data, currModel_ ));
	UndoRedo::instance().barrier( L("MODELEDITOR/MODELS/MUTANT/CHANGING_EDITOR_PROXY"), false );
...
void Mutant::removeEditorProxy()
{
	...
	UndoRedo::instance().barrier( L("MODELEDITOR/MODELS/MUTANT/REMOVING_EDITOR_PROXY"), false );
...
void Mutant::bumpMutant( DataSectionPtr visual, const std::string& primitivesName )
{
	if ( visual && !primitivesName.empty() )
	{
		...
		CopyFile( std::string(fullPath+".backup").c_str(), fullPath.c_str(), FALSE );

		DWORD att = GetFileAttributes( fullPath.c_str() );
		att &= ~FILE_ATTRIBUTE_READONLY;
		SetFileAttributes( fullPath.c_str(), att );
...
bool Mutant::loadModel( const std::string& name, bool reload )
{
	...
	superModel_->boundingBox( modelBB_ );
...
bool Mutant::addModel( const std::string& name, bool reload /* = true */ )
{
	...
	UndoRedo::instance().barrier( L("MODELEDITOR/MODELS/MUTANT/ADDING_MODEL"), false );
...
{
	UndoRedo::instance().add( new UndoRedoOp( 0, extraModels_ ));
	UndoRedo::instance().barrier( L("MODELEDITOR/MODELS/MUTANT/REMOVING_ADDED"), false );
...
void Mutant::recreateModelVisibilityBox( AnimLoadCallbackPtr pAnimLoadCallback, bool undoable )
{
	...
	if (undoable)
	{
		UndoRedo::instance().barrier( L("MODELEDITOR/MODELS/MUTANT/UPDATING_VIS_BOX"), false );
	}
...
void Mutant::saveCorrectPrimitiveFile( bool isSaving, bool isForcedSave )
{
	...
	{
		DWORD att = GetFileAttributes( backupPath.c_str() );
		if ( att & FILE_ATTRIBUTE_READONLY )
		{
			att &= ~FILE_ATTRIBUTE_READONLY;
			SetFileAttributes( backupPath.c_str(), att );
		}
	...
	else if ( isForcedSave )
	{
		...
		std::string fileSave;
		CopyFile( primPath.c_str(), backupPath.c_str(), false );
		fileSave = "Saving file: "+primitivesName_+"\n";
		ME_INFO_MSG( fileSave.c_str() );
		modelBeenBumped_ = false;
	}
	else
	{
...
void Mutant::save()
{
	...
	for ( std::set<DataSectionPtr>::iterator i = dirty_.begin(); i != dirty_.end(); i++ )
	{
		...
		{
			ME_INFO_MSG( L("MODELEDITOR/MODELS/MUTANT/SAVING", dataFiles_[*i]) );
		}
	...
	if( modelName_ != "" )
	{
		HANDLE mailSlot = CreateFile( "\\\\.\\mailslot\\WorldEditorUpdate",
...
bool Mutant::saveAs( const std::string& newName )
{
	...
	CopyFile( oldPrimitivesName.c_str(), newPrimitivesName.c_str(), FALSE );
