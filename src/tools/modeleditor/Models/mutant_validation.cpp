...
bool Mutant::locateFile( std::string& fileName, std::string modelName, const std::string& ext, const std::string& what, const std::string& criticalMsg /* = "" */ )
{
	...
	int response = ::MessageBox( AfxGetApp()->m_pMainWnd->GetSafeHwnd(),
		L("MODELEDITOR/MODELS/MUTANT_VALIDATION/CANT_LOCATE_Q", ext, fileName, ext, criticalMsg ), 
		L("MODELEDITOR/MODELS/MUTANT_VALIDATION/UNABLE_LOCATE", ext), MB_ICONERROR | MB_YESNO );
	...
	if (response == IDYES)
	{
		...
		BWFileDialog fileDlg ( TRUE, "", oldFileName.c_str(), OFN_FILEMUSTEXIST | OFN_HIDEREADONLY, fileFilter.c_str() );
...
{
	std::string fullPath = BWResource::resolveFilename( file );
	DWORD att = GetFileAttributes( fullPath.c_str() );
	if (att & FILE_ATTRIBUTE_READONLY)
	{
		::MessageBox( AfxGetApp()->m_pMainWnd->GetSafeHwnd(),
			L( "MODELEDITOR/MODELS/MUTANT_VALIDATION/READ_ONLY", file ), 
			L( "MODELEDITOR/MODELS/MUTANT_VALIDATION/READ_ONLY_TITLE" ),
	...
	if (testModel)
	{
		...
		if (testRoot != "")
		{
			...
			if (!testVisual)
			{
				...
				if (!locateFile( testRoot, name, "visual", what, L("MODELEDITOR/MODELS/MUTANT_VALIDATION/UNABLE_LOAD_OTHERWISE") ))
					return false;
