...
void MaterialPreview::update()
{
	...
	std::string textureString = MeApp::instance().mutant()->materialPropertyVal(
		PageMaterials::currPage()->materialName(),
		PageMaterials::currPage()->matterName(),
		PageMaterials::currPage()->tintName(),
		"diffuseMap",
		"Texture" );
	...
	if ( textureString != "")
	{
		HRESULT hr = D3DXSaveTextureToFile( fileName.c_str(), D3DXIFF_BMP,
