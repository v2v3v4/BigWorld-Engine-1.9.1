...
bool MeModule::init( DataSectionPtr pSection )
{
	...
	if (!pCaster_->init( pShadowCommon_, false ))
	{
...
void MeModule::render( float dTime )
{
	...
	{	
		...
		if (dTime != 0.f)
		{
			fps = L("MODELEDITOR/APP/ME_MODULE/POSITIVE_FPS", Formatter( 1.f/dTime, "%.1f" ) );
		}
		else
		{
			fps = L("MODELEDITOR/APP/ME_MODULE/ZERO_FPS");
		}
		...
		{
			CMainFrame::instance().setStatusText( ID_INDICATOR_TRIANGLES, L("MODELEDITOR/APP/ME_MODULE/TRIANGLES", numTris) );
...
bool MeModule::handleMouseEvent( const MouseEvent & event )
{
	...
	if ( InputDevices::isKeyDown( KeyEvent::KEY_SPACE ) && ( event.dz() != 0 ) )
	{
