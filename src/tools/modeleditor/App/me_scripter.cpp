...
static PyObject * py_undo( PyObject * args )
{
	...
	if (forStep < 0)
	{
		ME_INFO_MSG( L("MODELEDITOR/APP/ME_SCRIPTER/UNDOING", what ) );
...
static PyObject * py_redo( PyObject * args )
{
	...
	if (forStep < 0)
	{
		ME_INFO_MSG( L("MODELEDITOR/APP/ME_SCRIPTER/REDOING", what ) );
...
static PyObject * py_makeThumbnail( PyObject * args )
{
	...
	if ( strcmp( filePath, "") )
	{
		...
		{
			ME_INFO_MSG( L("MODELEDITOR/APP/ME_SCRIPTER/MAKE_THUMBNAIL",  MeApp::instance().mutant()->modelName() ) );
		}
	}
	else
	{
		ME_WARNING_MSG( L("MODELEDITOR/APP/ME_SCRIPTER/THUMBNAIL_ERROR") );
	}
