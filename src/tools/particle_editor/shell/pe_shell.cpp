...
PeShell::init( HINSTANCE hInstance, HWND hWndInput, HWND hWndGraphics )
{
    ...
    // romp needs chunky
    {
        ...
        ChunkDirMapping* mapping = space_->addMapping( SpaceEntryID(), (float*)nonConstIdentity, spacePath );
        if (!mapping)
        {
...
bool PeShell::initConsoles()
{
	...
	mgr.add( new ResourceUsageConsole( &ResourceStatistics::instance() ),	"Resource",	KeyEvent::KEY_F5, MODIFIER_CTRL | MODIFIER_SHIFT );
