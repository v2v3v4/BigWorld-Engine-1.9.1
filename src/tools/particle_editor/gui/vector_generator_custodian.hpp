...
void VectorGeneratorCustodian<CL>::updateControl()
{
    ...
    std::string generatorNameGUI = VecGenGUIStrFromID(generatorNameID);
    ...
    if (selectedString != CString(generatorNameGUI.c_str()))
    {
        // select string
...
void VectorGeneratorCustodian<CL>::updateOwner( bool drawIt  /*= true*/)
{
    ...
    if (curSel >= 0)
    {
        ...
        selectedStringID = VecGenIDFromGuiStr(selectedStringGUI.GetBuffer()).c_str();
    }
    std::string ownerId  = ((*owner_).*getFn_)()->nameID().c_str();
    std::string ownerGUI = VecGenGUIStrFromID(ownerId);

    if (selectedStringID != ownerId.c_str())
    {
        // create a new generator (delete is handled in the setPosition...
        VectorGenerator * newGenerator = 
            VectorGenerator::createGeneratorOfType(selectedStringID.GetBuffer());
        ...
        if ( x2_ )
        {
            if 
            (
                std::string(selectedStringID.GetBuffer()) == PointVectorGenerator::nameID_ 
                ||
                std::string(selectedStringID.GetBuffer()) == SphereVectorGenerator::nameID_
            )
            {
                x2_->EnableWindow( false );
                y2_->EnableWindow( false );
...
void VectorGeneratorCustodian<CL>::populate()
{
    ...
    comboBox_->AddString
    (
        VecGenGUIStrFromID(PointVectorGenerator::nameID_).c_str()
    );
    comboBox_->AddString
    (
        VecGenGUIStrFromID(LineVectorGenerator::nameID_).c_str()
    );
    comboBox_->AddString
    (
        VecGenGUIStrFromID(CylinderVectorGenerator::nameID_).c_str()
    );
    comboBox_->AddString
    (
        VecGenGUIStrFromID(SphereVectorGenerator::nameID_).c_str()
    );
    comboBox_->AddString
    (
        VecGenGUIStrFromID(BoxVectorGenerator::nameID_).c_str()
    );
